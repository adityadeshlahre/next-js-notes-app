...
- [x] Live URL fully functional: auth, notes, tags, filtering in production — **verified 2026-08-06**: `https://njnapp.onrender.com/` 200 (new landing), sign-in/email 200 + `__Secure-better-auth.session_token` cookie, `/dashboard` with cookie 200, `/api/notes` 200, freshly signed-in demo cookies round-trip dashboard. Demo account `demo@notesapp.dev` / `DemoPass123!` sign-in + dashboard verified live (curl). P0 note revised 2026-08-07: the stale/invalid-cookie deadlock machinery (server-side strip + client retry) was deliberately removed as overkill — login does a fresh server-side `getSession` and redirects; an old browser cookie may 403 a single sign-in and self-expires within better-auth's session TTL. Not re-verified with the current client-fetch/TanStack build after deploy.
